...
class VersionIndependentUtilsImplementation(loader: VersionIndependantLoader) : BaseVersionIndependentUtilImplementation1_17Plus(loader, DyeColorToMaterialMapper_1_17()) {
	...
	override fun getPlayerSkullWithBase64Texture(b64stringtexture: String): ItemStack {
		val profile = GameProfile(UUID.randomUUID(), null)
		val propertyMap = profile.properties ?: throw IllegalStateException("Profile doesn't contain a property map")
		propertyMap.put("textures", Property("textures", b64stringtexture))
		val head = ItemStack(Material.PLAYER_HEAD, 1)
		val headMeta = head.itemMeta
		val headMetaClass: Class<*> = headMeta!!.javaClass
		try {
			getField(headMetaClass, "profile", GameProfile::class.java, 0)[headMeta] = profile
	...
	override fun canBreakBlock(itemStack: ItemStack, material: Material): Boolean {
		...
		try {
			...
			for (nbt in f[list] as List<NBTTagString>) {
				val b = getMaterialFromName(nbt.e_()) == material
				if (b) {
					return true
				}
			}
	...
	override fun spawnFallingBlock(location: Location, material: Material, data: Byte, consumer: Consumer<FallingBlock>?): FallingBlock {
		val fb = EntityFallingBlock((location.world as CraftWorld?)!!.handle, location.x, location.y, location.z, CraftMagicNumbers.getBlock(material).n())
		fb.b = 1
	...
	override fun getBlockBlastResistance(material: Material): Float {
		return if (material.isBlock) {
			...
			try {
				val str = BlockBase::class.java.getDeclaredField("aI")
