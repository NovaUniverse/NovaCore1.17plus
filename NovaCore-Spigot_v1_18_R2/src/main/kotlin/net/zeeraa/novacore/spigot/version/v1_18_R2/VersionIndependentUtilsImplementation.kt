...
class VersionIndependentUtilsImplementation(loader: VersionIndependentLoader?) : BaseVersionIndependentUtilImplementation1_17Plus(loader!!, DyeColorToMaterialMapper_1_17()) {
	...
	override fun resetEntityMaxHealth(livingEntity: LivingEntity) {
		livingEntity.getAttribute(Attribute.GENERIC_MAX_HEALTH)!!.baseValue = livingEntity.getAttribute(Attribute.GENERIC_MAX_HEALTH)!!.defaultValue
	}
	...
	override fun spawnFallingBlock(location: Location, material: Material, data: Byte, consumer: Consumer<FallingBlock>?): FallingBlock {
		try {
			val fb = EntityFallingBlock.fall((location.world as CraftWorld?)!!.handle, BlockPosition(location.x, location.y, location.z), CraftMagicNumbers.getBlock(material).n(), CreatureSpawnEvent.SpawnReason.CUSTOM)
