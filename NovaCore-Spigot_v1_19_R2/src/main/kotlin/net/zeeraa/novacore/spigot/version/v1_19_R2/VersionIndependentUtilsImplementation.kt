...
class VersionIndependentUtilsImplementation(loader: VersionIndependentLoader?) : BaseVersionIndependentUtilImplementation1_17Plus(loader!!, DyeColorToMaterialMapper_1_17()) {
		...
		return when (e.cause) {
			...
			EntityDamageEvent.DamageCause.MAGIC -> {
				...
				if (lastDamager != null) {
					if (e is EntityDamageByEntityEvent) {
						val entityEvent = e
						if (entityEvent.damager is ThrownPotion) {
							val potion = entityEvent.damager as ThrownPotion
	...
	override fun setMarker(stand: ArmorStand, marker: Boolean) {
		stand.isMarker = marker
	}

	override fun isMarker(stand: ArmorStand): Boolean {
